...
static CGFloat const SW_ImageWidth = 150.0f;
...
- (void)layoutSubviews {
    ...
    self.icon.frame = CGRectMake(self.frame.size.width - 118, SW_EdgeMargin + 2, 50, 50);
...
- (UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section {
    return UIEdgeInsetsMake(0, 60, 10, 10);
}
...
+ (CGFloat)heightWithItem:(SWFormItem *)item {
    ...
    return item.images.count > 0 ? item.defaultHeight + 10*(rows+1) + rows*SW_ImageWidth:item.defaultHeight+40;
}
